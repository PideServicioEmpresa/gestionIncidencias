...
public sealed class CambiarRolCommandHandler : ICommandHandler<CambiarRolCommand>
{
    ...
    public async Task<Result> Handle(CambiarRolCommand request, CancellationToken ct)
    {
        ...
        if (actorDb.Rol is not (RolTipo.ADMIN or RolTipo.SUPERADMIN))
            return Result.NoPermitido("Solo administradores pueden cambiar roles de usuarios.");

        if (request.NuevoRol == RolTipo.SUPERADMIN && actorDb.Rol != RolTipo.SUPERADMIN)
            return Result.NoPermitido("Solo un SuperAdministrador puede asignar el rol SUPERADMIN.");

        if (request.UsuarioId == actorDb.Id)
            return Result.NoPermitido("No puede cambiar su propio rol.");
        ...
        try
        {
            var rolAnterior = usuario.Rol.ToString();
            ...
            await _auditService.RegistrarAsync(
                ...
                accion: "CambiarRol",
                antes: new { Rol = rolAnterior },
                despues: new { Rol = usuario.Rol.ToString() },
