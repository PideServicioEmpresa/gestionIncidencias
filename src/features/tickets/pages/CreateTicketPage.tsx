...
export function CreateTicketPage() {
  ...
  // Error visible junto al botón de enviar. A diferencia del toast, no desaparece solo
  // y está donde el usuario ya está mirando cuando pulsa Enviar.
  const [formError, setFormError] = useState<string | null>(null)
  const correosJefeRef = useRef<HTMLDivElement>(null)
  ...
  // Lleva a la vista el campo que impide enviar. React Hook Form ya enfoca el primer
  // error, pero solo en campos con ref nativo: los Select/SearchableSelect van por
  // Controller y el campo de correos ni siquiera pertenece al formulario.
  function desplazarAElemento(id: string) {
    const el = document.getElementById(id)
    el?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }

  // Se ejecuta cuando la validación del formulario falla al pulsar Enviar.
  const onInvalid = (errores: FieldErrors<CreateTicketForm>) => {
    setFormError('Revisa los campos marcados en rojo para poder enviar el ticket.')
    const primerCampo = Object.keys(errores)[0]
    if (primerCampo) desplazarAElemento(primerCampo)
  }
  ...
  const onSubmit = async (data: CreateTicketForm) => {
    ...
    if (requiereCorreosJefe && correosJefe.length === 0) {
      const mensaje = 'Agrega al menos un correo del jefe o supervisor.'
      setCorreosJefeError(mensaje)
      setFormError(mensaje)
      correosJefeRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' })
      return
    }
    ...
    try {
      ...
      // El mensaje queda fijo junto al botón: el toast se pierde si la pantalla
      // está desplazada hacia abajo, que es justo donde está el botón de enviar.
      setFormError(
  ...
  return (
    <div className="space-y-4 p-3 lg:p-5">
      ...
      <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-4">
        ...
        {/* ── Error de envío: fijo, junto al botón, no se cierra solo ──── */}
        {formError && (
          <div
            role="alert"
            aria-live="assertive"
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-destructive">No se pudo enviar el ticket</p>
              <p className="mt-0.5 text-xs leading-relaxed text-destructive/90">{formError}</p>
            </div>
          </div>
        )}
