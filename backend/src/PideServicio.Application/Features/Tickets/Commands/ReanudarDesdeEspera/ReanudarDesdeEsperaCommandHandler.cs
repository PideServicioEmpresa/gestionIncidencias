...
public sealed class ReanudarDesdeEsperaCommandHandler : ICommandHandler<ReanudarDesdeEsperaCommand>
{
    ...
    public ReanudarDesdeEsperaCommandHandler(
    ...
    public async Task<Result> Handle(ReanudarDesdeEsperaCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!esTecnicoAsignado && !tieneAccesoAdministrativo)
            return Result.NoPermitido("Solo el técnico asignado o un administrador puede reanudar el ticket.");

        var estadoAnterior = ticket.Estado;

        try
        {
            ticket.ReanudarDesdeEspera(actor.Id);
