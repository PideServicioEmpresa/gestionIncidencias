...
public sealed class DesactivarSucursalCommandHandler : ICommandHandler<DesactivarSucursalCommand, Guid>
{
    ...
    public async Task<Result<Guid>> Handle(DesactivarSucursalCommand request, CancellationToken ct)
    {
        ...
        if (actorDb.Rol is not (RolTipo.ADMIN or RolTipo.SUPERADMIN))
            return Result.NoPermitido<Guid>("Solo Administradores o SuperAdministradores pueden desactivar sucursales.");
        ...
        if (!sucursal.Activa)
            return Result.Fallo<Guid>("La sucursal ya está inactiva.");
        ...
        try
        {
            ...
            await _auditService.RegistrarAsync(
                "sucursales", sucursal.Id, "DESACTIVADO",
