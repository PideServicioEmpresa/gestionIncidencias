...
public sealed class PauseParaEsperaCommandHandler : ICommandHandler<PauseParaEsperaCommand>
{
    ...
    public PauseParaEsperaCommandHandler(
    ...
    public async Task<Result> Handle(PauseParaEsperaCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!esTecnicoAsignado && !tieneAccesoAdministrativo)
            return Result.NoPermitido("Solo el técnico asignado o un administrador puede pausar el ticket.");

        var estadoAnterior = ticket.Estado;

        try
        {
            ticket.PauseParaEspera(actor.Id);
            ...
            var historial = TicketHistorialEntrada.Crear(
                ...
                estadoNuevo: TicketEstadoTipo.EN_ESPERA);
